...
%==============================================================================
    ...
    tok_list = {}; next_pos = skip_whitespace(input_str, 1);
    while next_pos <= length(input_str)
        ...
        elseif (str_utils.is_digit(ch) || ch == '.')
            % number
            [toks, next_pos] = parse_number(input_str, next_pos);

        elseif ch == '"'
            % string literal
            [toks, next_pos] = parse_string(input_str, next_pos);
        ...
        elseif str_utils.ch_in_str(ch, punctuation_indicator_str)
            ...
            if isempty(tok_list) == false
                if is_semicolon(toks{1}) && is_semicolon(tok_list{end})
                    next_pos = skip_whitespace(input_str, next_pos);
                    continue;
                end
            end
        else
...
function new_next_pos = skip_whitespace(input_str, next_pos)
...
function [toks, new_macro_definitions, new_context_stack, new_next_pos] = ...
    ...
    next_pos = next_pos + 1;
    [identifier, next_pos] = parse_identifier(input_str, next_pos);
    toks = {};
...
function [out, new_next_pos] = parse_identifier(input_str, next_pos)
...
function [new_macro_definitions, new_next_pos] = ...
    ...
    % find macro details
    [macro.name, next_pos] = parse_identifier(input_str, next_pos);
    if input_str(next_pos) == '('
        macro.object_like = false;
        [macro.formal_arg_names, next_pos] = ...
                parse_formal_args(input_str, next_pos);
    else
        macro.object_like = true;
    end

    eol_pos = next_end_of_line(input_str, next_pos);
...
function [arg_names, new_next_pos] = ...
    ...
    while ~seen_close_paren
        ...
        if seen_close_paren && isempty(arg_name) && isempty(arg_names)
            % empty argument list
            break;
        elseif ~is_valid_identifier(arg_name)
...
function out = is_valid_identifier(s)
...
function eol_pos = next_end_of_line(input_str, next_pos)
...
function [toks, new_macro_definitions, new_next_pos] = ...
    ...
    [arg_toks, oof_1, oof_2] = ...
            frontend.VAPP_tokenize(include_arg, macro_definitions, context_stack, ...
                          identifier_replacements, parms);
    ...
    [toks, new_macro_definitions] = ...
            frontend.VAPP_pre_process(full_path, macro_definitions, parms);
...
function [toks, new_next_pos] = ...
    ...
    if macro.object_like
        ...
        for idx = 1:1:length(arg_strs)
            ...
            [arg_tok_array, oof_1, oof_2] = ...
                    frontend.VAPP_tokenize(arg_str, macro_definitions, context_stack, ...
                                  identifier_replacements, parms);
    ...
    [toks, oof_1, oof_2] = ...
            frontend.VAPP_tokenize(macro.body, macro_definitions, context_stack, ...
                          new_identifier_replacements, parms);
...
function match_idx = find_matching_double_quote(input_str, next_pos)
...
function match_idx = find_matching_bracket(input_str, next_pos)
...
function [toks, new_next_pos] = parse_number(input_str, next_pos)
...
function [toks, new_next_pos] = parse_string(input_str, next_pos)
