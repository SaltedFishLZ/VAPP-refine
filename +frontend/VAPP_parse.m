...
%==============================================================================
    ...
    while next_pos <= length(toks)
        ...
        elseif is_keyword(tok, 'module')
            ...
            if next_pos == -1
                AST = frontend.VAPP_AST_Node.empty;
                return;
            end
        ...
        elseif is_keyword(tok, 'analog')
            ...
            AST = frontend.VAPP_AST_Node.empty;
            frontend.VAPP_error('Unexpected token', tok);
            return;
        end
...
function test()
    ...
    [toks, oof_1, oof_2] = frontend.VAPP_tokenize(expr, {}, {}, {}, struct());
    AST = parse_expression(toks, uniqID_generator);

    disp(sprintf('Expression: %s', expr));
    disp('Expression AST: ');
    frontend.VAPP_print_AST(AST);
    %}
    ...
    [toks, oof_1, oof_2] = frontend.VAPP_tokenize(var_decl_stmt, {}, {}, {}, ...
                                         struct());
    [AST, new_next_pos] = ...
    ...
    [toks, oof_1, oof_2] = frontend.VAPP_tokenize(parm_decl_stmt, {}, {}, {}, ...
                                         struct());
    [AST, new_next_pos] = ...
    ...
    [toks, oof_1, oof_2] = frontend.VAPP_tokenize(contrib_stmt, {}, {}, {}, ...
                                         struct());
    [AST, new_next_pos] = ...
        parse_contribution_statement(toks, 1, uniqID_generator);
    ...
    [toks, oof_1, oof_2] = frontend.VAPP_tokenize(if_stmt, {}, {}, {}, struct());
    [AST, new_next_pos] = ...
    ...
    [toks, oof_1, oof_2] = frontend.VAPP_tokenize(case_stmt, {}, {}, {}, struct());
    [AST, new_next_pos] = ...
    ...
    [toks, oof_1, oof_2] = frontend.VAPP_tokenize(for_loop, {}, {}, {}, struct());
    [AST, new_next_pos] = ...
        parse_for_loop(toks, 1, contrib_allowed, uniqID_generator);
    ...
    [toks, oof_1, oof_2] = frontend.VAPP_tokenize(repeat_loop, {}, {}, {}, struct());
    [AST, new_next_pos] = ...
    ...
    [toks, oof_1, oof_2] = frontend.VAPP_tokenize(while_loop, {}, {}, {}, struct());
    [AST, new_next_pos] = ...
    ...
    [toks, oof_1, oof_2] = frontend.VAPP_tokenize(analog_func_defn, {}, {}, {}, ...
                                         struct());
    [AST, new_next_pos] = ...
        parse_analog_function(toks, 1, uniqID_generator);
    ...
    [toks, oof_1, oof_2] = frontend.VAPP_tokenize(module_defn, {}, {}, {}, struct());
    [AST, new_next_pos] = parse_module(toks, 1, uniqID_generator);
...
function out = is_one_of_the_keywords(tok, kw_arr)
...
function out = is_one_of_the_symbols(tok, sym_arr)
...
function out = is_one_of_the_symbols_or_keywords(tok, ...
    ...
    out = (    is_one_of_the_symbols(tok, sym_arr) ...
            || is_one_of_the_keywords(tok, kw_arr) );
...
function out = is_one_of_the_identifiers(tok, id_arr)
...
function out = is_subset(sub_cell, super_cell)
...
function out = convert_const_dtype(from_type, to_type, key)
    ...
    if ~success
        ...
        frontend.VAPP_error(['Not a valid constant ', err_type]);
        out = -1;
        return;
    end
...
function success = assert_keyword(tok, kw)
    ...
    if nargin == 1
        if ~is_keyword(tok)
            success = -1;
            frontend.VAPP_error('Expected a keyword token, but did not find it', tok);
        end
    else
        if ~is_keyword(tok, kw)
            success = -1;
            frontend.VAPP_error(sprintf('Expected keyword "%s", but did not find it', kw), tok);
        end
    end
end

function success = assert_one_of_the_keywords(tok, kw_arr)
    success = 0;
    if ~is_one_of_the_keywords(tok, kw_arr)
        s = str_utils.str_arr_to_str(kw_arr);
        success = -1;
        frontend.VAPP_error(['Expected one of the keywords ', s, ', but did not find it'], tok);
    end
end
...
function success = assert_symbol(tok, sym)
    ...
    if nargin == 1
        if ~is_symbol(tok)
            success = -1;
            frontend.VAPP_error('Expected a punctuation symbol token, but did not find it', tok);
        end
    else
        if ~is_symbol(tok, sym)
            success = -1;
            frontend.VAPP_error(sprintf('Expected symbol "%s", but did not find it', sym), tok);
        end
    end
end

function success = assert_one_of_the_symbols(tok, sym_arr)
    success = 0;
    if ~is_one_of_the_symbols(tok, sym_arr)
        success = -1;
        s = str_utils.str_arr_to_str(sym_arr);
        frontend.VAPP_error(['Expected one of the symbols ', s, ', but did not find it'], tok);
    end
end
...
function success = assert_one_of_the_symbols_or_keywords(tok, ...
                                               ...
                                               kw_arr)
    success = 0;
    if ~is_one_of_the_symbols_or_keywords(tok, sym_arr, kw_arr)

        success = -1;
        s_sym = str_utils.str_arr_to_str(sym_arr);
        s_kw = str_utils.str_arr_to_str(kw_arr);

        frontend.VAPP_error(['Expected one of the symbols ', s_sym, ...
...
% no need to use frontend.VAPP_error
function assert_subset(sub_cell, super_cell)
    if ~is_subset(sub_cell, super_cell)
...
function success = assert_identifier(tok, id_name)
    ...
    if nargin == 1
        if ~is_identifier(tok)
            success = -1;
            frontend.VAPP_error('Expected an identifier token, but did not find it', tok);
        end
    else
        if ~is_identifier(tok, id_name)
            success = -1;
            frontend.VAPP_error(sprintf('Expected identifier "%s", but did not find it', ...
                          id_name), tok);
        end
    end
end

function success = assert_one_of_the_identifiers(tok, id_arr)
    success = 0;
    if ~is_one_of_the_identifiers(tok, id_arr)
        s = str_utils.str_arr_to_str(id_arr);
        success = -1;
        frontend.VAPP_error(['Expected one of the identifiers ', s, ', but did not find it'], tok);
    end
end
...
function success = assert_simulator_directive(tok)
    ...
    if ~is_simulator_directive(tok)
        success = -1;
        frontend.VAPP_error('Expected a simulator directive token, but did not find it', tok);
    end
end
...
function success = assert_string(tok)
    ...
    if ~is_string(tok)
        success = -1;
        frontend.VAPP_error('Expected a string token, but did not find it', tok);
    end
end
...
% ASSERT_NUMERIC_CONSTANT
    ...
    if ~is_numeric_constant(tok)
        success = -1;
        frontend.VAPP_error('Expected a numeric constant token, but did not find it', tok);
    end
end
...
function new_next_pos = skip_to_keyword(kw, toks, next_pos)
    ...
    new_next_pos = -1;
    frontend.VAPP_error(['Keyword token "', kw, '" not found'], tok);
...
function new_next_pos = skip_to_symbol(sym, toks, next_pos)
    ...
    if isempty(tok) == false
        frontend.VAPP_error(['Symbol token "', sym, '" not found'], tok);
    else
        frontend.VAPP_error(['Symbol token "', sym, '" not found']);
    end

end

function new_next_pos = ...
            skip_to_one_of_the_symbols(sym_arr, toks, next_pos)
    ...
    s = str_utils.str_arr_to_str(sym_arr);
    new_next_pos = -1;
    frontend.VAPP_error(['None of the symbols ', s, ' found'], tok);
...
function new_next_pos = skip_to_one_of_the_symbols_or_keywords(sym_arr, ...
    ...
    s_kw = str_utils.str_arr_to_str(kw_arr);
    new_next_pos = -1;
    frontend.VAPP_error(['None of the symbols ', s_sym, ' or keywords ', s_kw, ' found'], tok);
...
% units, access, ddt_nature, idt_nature, abstol
    ...
    while true
        ...
        try
            ...
            new_next_pos = -1;
            frontend.VAPP_error('Expected keyword "endnature", but did not find it',...
                                                           toks{next_pos-1});
            return;
        end
...
% PARSE_DISCIPLINE
    ...
    while true
        try
            ...
            new_next_pos = -1;
            frontend.VAPP_error('Expected keyword "enddiscipline", but did not find it',...
                                                           toks{next_pos-1});
            return;
        end
        ...
        if is_keyword(tok, 'potential')
            ...
            if isempty(tok) == false
                ...
                try 
                    ...
                    success = -1;
                    next_pos = -1;
                    frontend.VAPP_error(['Tried to access the potential "', ...
        ...
        elseif is_keyword(tok, 'flow')
            ...
            if isempty(tok) == false
                ...
                try 
                    ...
                    success = -1;
                    next_pos = -1;
                    frontend.VAPP_error(['Tried to access the flow "', ...
                                nature_name, '" but could not find it. ',...
                                ' This nature has to be defined prior to',...
                                ' this discipline definition!'], tok);
                end
...
% return an empty token if the file ends here.
    ...
    try
        ...
        tok = [];
        new_next_pos = -1;
        frontend.VAPP_error(['Expected another token ', expected_token_paren ,...
...
% 2*1e-6) it will result in an error.
    ...
    if new_next_pos == -1
        tok = [];
        frontend.VAPP_error('Expected a LHS expression, but did not find it',...
                                                           toks{next_pos});
        return;
    end
...
function [AST, new_next_pos] = ...
    ...
    while true
        ...
        try
            ...
            AST = frontend.VAPP_AST_Node.empty;
            new_next_pos = -1;
            frontend.VAPP_error('Expected keyword "endmodule", but did not find it',...
                                                           toks{next_pos-1});
            return;
        end
        ...
        % frontend.VAPP_pre_process and frontend.VAPP_tokenize.
        %
        % OR
        %
        ...
        elseif is_one_of_the_identifiers(tok, discipline_arr)
            discipline_name = tok.value;
...
function [AST, new_next_pos] = ...
    ...
    [return_type, func_name, next_pos] = ...
        parse_analog_function_declaration(toks, next_pos);
...
function [AST_dtype, func_name, new_next_pos] = ...
    ...
    tok = toks{next_pos};
    if is_one_of_the_keywords(tok, {'integer', 'real'})
        VA_dtype = tok.value;
        next_pos = next_pos + 1;
    else
        VA_dtype = 'real';
...
function [ASTs, new_next_pos] = ... 
    ...
    if ~is_symbol(tok, '<') && ~is_identifier(tok)
        ...
        ASTs = {};
        new_next_pos = -1;
        frontend.VAPP_error('List of ports/identifiers cannot be empty', tok);
        return;
    end
    ...
    while true
        ...
        if next_pos == -1
            new_next_pos = -1;
            ASTs = frontend.VAPP_AST_Node.empty;
            return;
        end
...
function [AST, new_next_pos] = ...
    ...
    % the data type
    tok = toks{next_pos};
    success = assert_one_of_the_keywords(tok, {'integer', 'real', 'string'});
    ...
    while true
        ...
        tok = toks{next_pos};
        success = assert_one_of_the_symbols(tok, {',', ';'});
...
function [AST, new_next_pos] = parse_variable_declaration(AST_dtype, ...
    ...
    success = assert_one_of_the_symbols(toks{comma_or_semicolon_pos}, sym_arr);
...
function [AST, new_next_pos] = ...
    ...
    tok = toks{next_pos};
    success = assert_one_of_the_symbols_or_keywords(tok, sym_arr, kw_arr);
    ...
    while true
        ...
        if ~is_one_of_the_keywords(tok, {'from', 'exclude'})
            break;
        end

        [parm_range_AST, next_pos] = ...
            parse_parameter_range(toks, next_pos, access_arr, uniqID_generator);
    ...
    success = assert_one_of_the_symbols(tok, sym_arr);
...
function [AST, new_next_pos] = ...
    ...
    success = assert_one_of_the_keywords(toks{next_pos}, kw_arr);
    ...
    Type = detect_parm_range_Type(toks, next_pos);
...
function Type = detect_parm_range_Type(toks, next_pos)
    ...
    tok = toks{next_pos};
    success = assert_one_of_the_keywords(tok, kw_arr);
    ...
    tmp = skip_over_expression(toks, next_pos, sym_arr, kw_arr);
    if tmp ~= -1
        Type = 'exclude_value';
    else
        Type = 'exclude_interval';
...
function [AST, new_next_pos] = ...
    ...
    elseif is_symbol(tok, '''')
        ...
        new_next_pos = -1;
        AST = frontend.VAPP_AST_Node.empty;
        frontend.VAPP_error('Expected one of {"(", "[", "''"}, but did not find it', tok);
        return;
    end
...
function [AST, new_next_pos] = ...
    ...
    elseif is_symbol(tok, '[')
        ...
        new_next_pos = -1;
        AST = frontend.VAPP_AST_Node.empty;
        frontend.VAPP_error('Expected an "(" or "[", but did not find it', tok);
        return;
    end
    next_pos = next_pos + 1;
...
function [AST, new_next_pos] = parse_core_statement(toks, ...
    ...
    if is_identifier(tok)
        ...
        if is_symbol(tok2, '=')
            ...
            if ~contrib_allowed
                new_next_pos = -1;
                AST = frontend.VAPP_AST_Node.empty;
                frontend.VAPP_error('Contribution statements not allowed here', tok);
                return;
            end
            [AST, new_next_pos] = ...
    ...
    elseif is_symbol(tok, '@')
        ...
        new_next_pos = -1;
        AST = frontend.VAPP_AST_Node.empty;
        frontend.VAPP_error('Expected a "core" statement, but did not find it', tok);
        return;
    end
...
function [var_name, expr_AST, terminator_pos] = ...
    ...
    if success == -1
        var_name = '';
        terminator_pos = -1;
        expr_AST = frontend.VAPP_AST_Node.empty;
        return;
    end
...
function [AST, new_next_pos] = parse_case_statement(toks, ...
    ...
    if isempty(case_items_ASTs)
        new_next_pos = -1;
        AST = frontend.VAPP_AST_Node.empty;
        frontend.VAPP_error('Case statement has to contain at least one case item', tok);
        return;
    end
...
function [ASTs, new_next_pos] = ...
    ...
    while true
        ...
        ASTs = [ASTs, {AST}];
        success = assert_one_of_the_symbols(toks{comma_or_colon_pos}, sym_arr);
...
function [AST, new_next_pos] = ...
    ...
    success = assert_one_of_the_identifiers(toks{next_pos}, access_arr);
...
function [AST, success] = parse_expression(toks, access_arr, ...
    ...
    while next_pos <= length(toks)
        ...
        elseif is_expr_op(tok)
            ...
            op = resolve_ariness(tok, prev_tok);
            [op_stack, expr_stack] = ...
            ...
            AST = frontend.VAPP_AST_Node.empty;
            success = -1;
            frontend.VAPP_error('Unrecognized token while parsing expression', tok);
            return;
        end
    ...
    while ~isempty(op_stack)
        ...
        expr_stack = apply_op(op, expr_stack, uniqID_generator);
    end

    if length(expr_stack) ~= 1
        AST = frontend.VAPP_AST_Node.empty;
        success = -2;
        frontend.VAPP_error('Expression stack does not contain exactly one item');
        return;
    end
...
function [AST, terminator_pos] = ...
    ...
    if success ~= 0
        if success == -2
            frontend.VAPP_error('Expected another expression before token but did not find it', ...
                                                         toks{terminator_pos});
        end
        terminator_pos = -1;
        return;
...
function new_next_pos = ...
            skip_over_expression(toks, next_pos, sym_arr, kw_arr)
    ...
    assert_subset(sym_arr, {';', ')', ',', ':', '<+', ']'});

    % verify that kw_arr is a subset of {'from', 'exclude'}
    assert_subset(kw_arr, {'from', 'exclude'});
    ...
    while true
        ...
        elseif is_symbol(tok, ')')
            if isempty(stack)
                ...
                if top ~= '('
                    new_next_pos = -1;
                    frontend.VAPP_error('Mismatched parentheses', tok);
                    return;
                end
            end
        ...
        elseif is_symbol(tok, ':')
            if isempty(stack)
                ...
                if top ~= '?'
                    new_next_pos = -1;
                    frontend.VAPP_error('Mismatched conditional (?:) operator', tok);
                    return;
                end
            end
        ...
        elseif is_symbol(tok, ',')
            if isempty(stack)
                ...
                if top ~= '('
                    new_next_pos = -1;
                    frontend.VAPP_error('Improper use of comma in expression', tok);
                    return;
                end
            end
            ...
            if isempty(stack)
                ...
                new_next_pos = -1;
                frontend.VAPP_error('Syntax error in expression', tok);
                return;
            end
...
function [AST, new_next_pos] = parse_function_call(toks, next_pos,...
    ...
    if ~is_keyword(tok) && ~is_identifier(tok)
        frontend.VAPP_error('Expected a keyword/identifier, but did not find it', tok);
        AST = frontend.VAPP_AST_Node.empty;
        new_next_pos = -1;
        return;
    end
...
function [ASTs, new_next_pos] = ...
    ...
    while true
        ...
        ASTs = [ASTs, {AST}];
        success = assert_one_of_the_symbols(toks{comma_or_close_paren_pos}, sym_arr);
...
function new_expr_stack = apply_op(op, expr_stack, uniqID_generator)

    ariness = get_ariness(op);
...
function out = is_expr_op(tok)

    ops = { '!', '**', '*', '/', '%', '+', '-', '<', '<=', '>', '>=', ...
            '==', '!=', '&&', '||', '?', ':' };

    out = is_one_of_the_symbols(tok, ops);

end

function op = resolve_ariness(tok, prev_tok)
...
function [new_op_stack, new_expr_stack] = ...
    ...
    [op_prec, op_assoc] = get_precedence_and_associativity(op);
    ...
    while true
        ...
        [top_prec, oof] = get_precedence_and_associativity(top);
        ...
        expr_stack = apply_op(top, expr_stack, uniqID_generator);
...
function [prec, assoc] = get_precedence_and_associativity(op)
    ...
    prec = -1;
    assoc = -1;
    frontend.VAPP_error(sprintf('Operator %s not found in precedence table', op));

end

function out = get_ariness(op)
...
function AST = create_root_AST(children, uniqID_generator)
    ...
    AST = frontend.VAPP_AST_Node(uniqID_generator);
    AST.set_type('root');
